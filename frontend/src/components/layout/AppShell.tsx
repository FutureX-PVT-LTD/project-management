...
export function AppShell({
  ...
  return (
    <div className="min-h-screen bg-fx-bg flex">
      ...
      {/* Sidebar (Desktop 208px / Mobile Drawer) */}
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-50 w-[208px] bg-white border-r border-fx-border flex flex-col justify-between transition-transform duration-200 ease-in-out lg:translate-x-0',
          mobileOpen ? 'translate-x-0 shadow-drawer' : '-translate-x-full',
        )}
      >
        <div className="flex flex-col h-full">
          {/* Workspace Brand / Header */}
          <div className="h-[56px] px-3.5 border-b border-fx-border/70 flex items-center justify-between">
                ...
                {navigation.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      onClick={() => setMobileOpen(false)}
                      className={cn(
                        'h-[36px] flex items-center justify-between px-2.5 rounded-lg text-xs font-medium fx-transition',
                        isActive
                  ...
                  {adminNav.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        onClick={() => setMobileOpen(false)}
                        className={cn(
                          'h-[36px] flex items-center gap-2.5 px-2.5 rounded-lg text-xs font-medium fx-transition',
                          isActive
                            ? 'bg-fx-green-soft text-fx-green-dark font-semibold'
                            : 'text-fx-text-secondary hover:text-fx-text-primary hover:bg-fx-bg-hover',
                        )}
          ...
          <div className="p-2 border-t border-fx-border/70 bg-white">
            <ProfilePopover>
              <button
                type="button"
                className="w-full flex items-center justify-between p-1.5 rounded-lg hover:bg-fx-bg-hover fx-transition text-left group"
              >
                <div className="flex items-center gap-2 min-w-0">
                  <div className="w-7 h-7 rounded-full bg-fx-green-soft text-fx-green-dark border border-fx-green/20 font-semibold text-[11px] flex items-center justify-center shrink-0">
                    {initials}
                  </div>
      ...
      {/* Main Content Area */}
      <div className="flex-1 flex flex-col min-w-0 lg:pl-[208px]">
        {/* Top Bar (56px) */}
        <header className="h-[56px] bg-white border-b border-fx-border px-4 sm:px-6 lg:px-8 flex items-center justify-between sticky top-0 z-30">
          ...
          <div className="flex items-center gap-3">
            {/* Global Search trigger (Ctrl+K ready, 280-340px width) */}
            <div className="relative hidden md:block">
              <Link
                href="/my-work"
                className="w-72 h-8 px-2.5 bg-fx-bg text-xs text-fx-text-muted rounded-lg border border-fx-border hover:border-fx-border-strong flex items-center justify-between fx-transition"
              >
            ...
            <Link
              href="/notifications"
              className="relative p-2 text-fx-text-secondary hover:text-fx-text-primary hover:bg-fx-bg-hover rounded-lg fx-transition"
        ...
        {/* Main Content Workspace Canvas (Fluid 88-94% width) */}
        <main
          className={cn(
            'flex-1 p-6 sm:p-7 lg:p-8',
            fullWidth ? 'max-w-full' : 'max-w-[1720px] mx-auto w-full',
          )}
        >
          {children}
        </main>
