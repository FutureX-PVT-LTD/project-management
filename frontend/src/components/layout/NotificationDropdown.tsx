        ...
        >
          <div className="flex items-center justify-between px-4 py-3 border-b border-fx-border">
            <div className="flex items-center gap-2">
              ...
              {unreadCount > 0 && (
                <span className="text-xs bg-fx-green-soft text-fx-green font-semibold px-2 py-0.5 rounded-full">
                  {unreadCount} new
                </span>
              )}
            </div>
            {unreadCount > 0 && (
              <button
                ...
                className="text-xs text-fx-green hover:underline flex items-center gap-1 font-medium"
              >
          ...
          <div className="max-h-80 overflow-y-auto divide-y divide-fx-border/50">
            ...
            {notifications.slice(0, 10).map((n: any) => (
              ...
              >
                ...
                {!n.isRead && (
                  <span className="h-2 w-2 rounded-full bg-fx-green mt-1 shrink-0" />
                )}
              </div>
            ))}
          </div>
